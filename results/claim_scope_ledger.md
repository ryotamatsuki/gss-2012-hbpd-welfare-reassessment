...
| CS-WEAK | branch-correct weak-HBP profile CS; equilibrium only on pure overlap | source Eq. (13)–(17) recertification + primitive integral + symbolic reconstruction | MODEL-SPECIFIC | Eq. (15) is correct on switching branch; Eq. (16) reverses its sign; weak source CS reversal does not survive branch-correct integration | claim that Eq. (15) itself is wrong; corrected equilibrium CS over no-pure region |
...
| CS-STRONG | nondegenerate (x_0<1): source/nonnegative-margin strong-HBP selection and corrected pure-uniform domain; endpoint (x_0=1) separate | Stage 7 + strong-HBP selection/empty-segment audit | MODEL-SPECIFIC / SELECTION-CONDITIONAL | Result 6 survives for (x_0<1) under source selection; CS gap is zero at (x_0=1) | full price-vector uniqueness at (x_0=1); selection-free CS ranking with unrestricted below-cost prices |
