...
| CS-WEAK | branch-correct weak-HBP profile CS; equilibrium only on pure overlap | primitive integral, source-image fidelity check, symbolic reconstruction, exact regressions | MODEL-SPECIFIC | accepted Eq. (15) is valid on its switching branch; Eq. (16) sign and no-switch branch must be separated; weak source CS reversal does not survive | claim that Eq. (15) itself is algebraically wrong; corrected equilibrium CS over no-pure region |
| PROFIT-B-WEAK | small-firm HBP profit comparison on weak pure overlap; profile comparisons outside it are branch-specific | Stage 7 analytic sign proof + exact no-switch regression | MODEL-SPECIFIC | on every weak-domain pure-equilibrium comparison, \(\pi_B^d<\pi_B^u\) | extension to strong dominance; use of switching-branch Eq. (22) as a branch-unrestricted profile formula; statement about mixed-equilibrium region |
...
| CS-STRONG | source/nonnegative-margin strong-HBP selection on corrected pure-uniform domain for `x_0<1`; empty B-history endpoint handled separately | Stage 7 + strong-HBP selection/endpoint audit | MODEL-SPECIFIC / SELECTION-CONDITIONAL | Result 6 survives under source selection for `x_0<1`; gap is zero at `x_0=1` | unique four-price vector at `x_0=1`; selection-free CS ranking with unrestricted below-cost prices |
