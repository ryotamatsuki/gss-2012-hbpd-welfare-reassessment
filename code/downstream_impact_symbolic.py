...
assert sp.factor(cs_weak_switch_num.subs(x, xu)) ==     2 * (25*s**2 - 72*s + 63) / 9
assert sp.factor(cs_weak_switch_num.subs(x, xbar)) ==     -(s + 1) * (45*s - 43) / 4
assert sp.factor(cs_weak_noswitch_num.subs(x, sp.Rational(1, 2))) ==     (s - 7) * (s - 1)
assert sp.factor(cs_weak_noswitch_num.subs(x, xu)) ==     (25*s**2 - 72*s + 63) / 9
assert sp.factor(cs_weak_noswitch_num.subs(x, xbar)) ==     -(s - 1) * (5*s + 13) / 2
...
assert sp.factor((18*pi_b_weak_gap).subs(x, sp.Rational(1, 2))) ==     s**2 + 8*s - 4
assert sp.factor((18*pi_b_weak_gap).subs(x, xbar)) ==     (s + 1) * (21*s - 11) / 4
assert sp.diff(18*pi_b_weak_gap, x, 2) == 40
...
assert sp.factor(w_weak_switch.subs(x, xbar)) ==     -(1+s)*(1+9*s)/144
