...
\begin{table}[t]
\centering
\caption{Exact global-deviation counterexample at \((\tau,\sigma,x_0,c)=(1,1/2,3/5,0)\)}
...
\addplot+[thick,mark=none] table[x=s,y=x_H,col sep=comma]{generated/parameter_domains.csv};
\addlegendentry{\(x_H(s)\): pure-uniform threshold}
\addplot+[dashed,thick,mark=none] table[x=s,y=x_bar,col sep=comma]{generated/parameter_domains.csv};
...
\addplot+[densely dashdotted,mark=none] coordinates {(0.1333978072,0.5) (0.1333978072,0.7166505482)};\n\addlegendentry{\(s_c\): weak/pure overlap endpoint}\n\addplot+[only marks,mark=*,mark size=2.4pt] coordinates {(0.1333978072,0.7166505482)} node[above right] {\(s_c\)};
\end{axis}
\end{tikzpicture}
\caption{Corrected equilibrium and source-branch boundaries. The weak-HBP/pure-uniform equilibrium overlap lies between \(x_H(s)\) and \(\bar x(s)\) and exists only for \(s<s_c\).}
\label{fig:domains}
\end{figure}
