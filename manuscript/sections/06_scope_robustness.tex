...
Proposition~\ref{prop:uniform} is unchanged whether uniform prices are allowed on all of \(\mathbb R\) or restricted to \(p_i\ge c\). A negative-margin price with positive demand yields negative profit and can be replaced by \(c\), while a zero-margin price with positive demand can be raised slightly without losing all demand. Full-capture profiles are excluded by a joint margin--demand argument: if the capturing firm has a nonpositive margin it can improve its own price, whereas with a positive capture margin the excluded firm can cross the capture boundary at a positive margin. This reduction is specific to the uniform-price game and should not be silently transferred to the history-based game.
