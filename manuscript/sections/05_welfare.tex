...
\label{eq:strong-cs}
\end{equation}
Therefore the accepted strong-dominance consumer-surplus threshold survives under the source HBP selection when poaching margins are constrained to be nonnegative, conditional on the corrected uniform pure-existence condition.
...
4 & Smaller-firm profit & Accepted Eq.~(22) is itself switching-branch-specific. On the corrected weak pure-equilibrium overlap \(\pi_B^d<\pi_B^u\); outside it, the counterfactual profile comparison must use realized branch demand. \\
...
6 & Strong consumer surplus & For \(x_0<1\), survives on the corrected pure domain under the source/nonnegative-margin strong-HBP selection; unrestricted below-cost pricing introduces selection dependence. At \(x_0=1\), the difference is zero and unused HBP prices are indeterminate. \\
