...
\frac{(1+s)(21s-11)}4<0
\]
for \(0\le s<s_c\). Convexity then implies negativity throughout the interval. Outside the pure overlap, accepted Eq.~(22) cannot simply be relabeled a profile comparison. When (x_0<x_u), the second branch in eqref{eq:B-profit-gap} must be used. For example,
[
(s,x_0)=left(rac12,rac{51}{100}ight)
]
gives
[
pi_B^d=rac{3221}{9000}	au,qquad
pi_B^u=rac{49}{120}	au,qquad
pi_B^d-pi_B^u=-rac{227}{4500}	au<0,
]
whereas extending accepted Eq.~(22) into this no-switch region would give (4	au/375>0). Result~4 therefore requires branch-correct profile accounting outside the equilibrium overlap.
